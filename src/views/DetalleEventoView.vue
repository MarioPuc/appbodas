    ...
    <div class="container">
      ...
      <div v-if="logged === 'Logged'">
        ...
        <v-container>
          ...
          <v-form ref="form" v-model="statusValido" lazy-validation class="py-10 text-right">
            ...
            <v-row>
              ...
              <v-col cols="6">
                <v-file-input id="campoDocs" accept=".pdf, .jpg" small-chips multiple
                  ...
                  :show-size="1000" prepend-icon="mdi-file-document-plus" :error="docsError"
                  @change="subirImagenes()"></v-file-input>

                <ul>
                  <li v-for="item of evento.docs">
                    <a :href="'https://workspacedigiart.com/helpers/' + item.name" target="_blank">
                      {{ item.originalName }}</a>
                  </li>
                </ul>
              </v-col>
          ...
          <v-card class="px-8 py-6">
            ...
            <div class="grid grd-cols-2 text-right">
              <div></div>
              <div>
                <v-btn class="red white--text mx-3" v-on:click="() => {
                  isSetAgregarInvitados = true;
                  isSetImportarInvitados = false;
                }">Agregar Invitado</v-btn>

                <v-btn class="success mx-3" v-on:click="() => {
            ...
            <div v-if="isSetAgregarInvitados">
              <v-form ref="form" v-model="statusValidoInvitado" lazy-validation class="py-10 text-right">
                <v-row class="py-4">
                  <v-col cols="4">
                    <v-text-field label="Nombre del invitado" hide-details="auto"
                      v-model="invitado.nombreInvitado"></v-text-field>
                  </v-col>

                  <v-col cols="4">
                    <v-text-field label="Teléfono del invitado" hide-details="auto" v-model="invitado.telefonoInvitado"
                      :rules="reglas.telefonoInvitado"></v-text-field>
                  </v-col>

                  <v-col cols="4">
                    <v-text-field label="Correo electrónico" hide-details="auto" v-model="invitado.email"
                      :rules="reglas.email"></v-text-field>
                  </v-col>

                  <v-col cols="4">
                    <v-text-field label="Total asistentes" type="number" min="1" hide-details="auto"
                      v-model="invitado.totalAsistentes" :rules="reglas.totalAsistentes"></v-text-field>
                  </v-col>

                  <v-col cols="4">
                    <v-text-field label="Número de mesa" type="number" min="1" hide-details="auto"
                      v-model="invitado.numeroMesa"></v-text-field>
                  </v-col>
                </v-row>
            ...
            <div v-if="isSetImportarInvitados">
              <v-form>
                <v-file-input label="Subir archivo xlsx" v-model="archivoXLSX" @change="processFile()">
                </v-file-input>
              </v-form>
            </div>

            <template>
              <v-data-table :headers="headers" :items="this.invitados" :items-per-page="5" class="elevation-1">
                <template v-slot:item.code="{ item }">
                  <input v-on:focus="$event.target.select()" :ref="'code' + item.idInvitado" class="input-clone"
                    readonly :value="'https://workspacedigiart.com/invitado/' + item.idInvitado" />
                  <v-icon small color="info" class="mr-2" @click="clipboard('code' + item.idInvitado)">
                    mdi-content-copy
                  </v-icon>
                </template>
                <template v-slot:item.whatsCode="{ item }">
                  <input v-on:focus="$event.target.select()" :ref="'whatsCode' + item.idInvitado" class="input-clone"
                    readonly
                  ...
                  <v-icon small color="info" class="mr-2" @click="clipboard('whatsCode' + item.idInvitado)">
                    mdi-content-copy
                  </v-icon>
                </template>
                ...
                <template v-slot:item.actions="{ item }">
                  <v-icon small color="info" class="mr-2" v-on:click="editandoInvitado(item)">
                    mdi-pencil
                  </v-icon>
                  <v-icon small color="info" class="mr-2" v-on:click="enviarCorreoInvitado(item)">
                    mdi-email
                  </v-icon>
                  <v-icon small color="error" class="d-none">
                    mdi-delete
                  </v-icon>
                </template>
              </v-data-table>
